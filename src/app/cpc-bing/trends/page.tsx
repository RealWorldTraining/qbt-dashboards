...
interface BingTrendsData {
  trends: Record<string, Record<string, BingKeywordWeek[]>>
  lastUpdated: string
}
...
const metricConfigs = [
  ...
  { key: 'avgCpc', label: 'Avg CPC', color: '#10B981' },
  { key: 'cost', label: 'Cost', color: '#F59E0B' },
  { key: 'conversions', label: 'Conversions', color: '#8B5CF6' },
  { key: 'convRate', label: 'Conv Rate %', color: '#EF4444' },
  { key: 'cpa', label: 'CPA', color: '#06B6D4' }
]

export default function BingCPCTrendsPage() {
  const [data, setData] = useState<BingTrendsData | null>(null)
  const [loading, setLoading] = useState(true)
  const [selectedTab, setSelectedTab] = useState(campaignTabs[0])

  useEffect(() => {
    fetch('/api/cpc-bing-trends')
      .then(res => res.json())
      .then(setData)
      .catch(console.error)
  ...
  const allKeywords: Record<string, BingKeywordWeek[]> = {}
      ...
      <div className="max-w-[1800px] mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-white text-2xl font-bold">Bing Ads - CPC Keyword Trends</h1>
          </div>
        ...
        <div className="mb-4 text-gray-400">
          Showing {keywordNames.length} Bing Ads keywords for {selectedTab}
        </div>
                  ...
                  data={{
                    ...
                    datasets: keywordNames.map((kw, i) => {
                      ...
                      return {
                        ...
                        data: weeks.map(w => w[metric.key as keyof BingKeywordWeek] as number),
                    ...
                    scales: {
                      ...
                      y: {
                        ...
                        ticks: { 
                          ...
                          font: { size: 10 },
                          callback: (value: any) => {
                            if (metric.key === 'avgCpc' || metric.key === 'cost' || metric.key === 'cpa') {
                              return '$' + value.toFixed(2)
                            } else if (metric.key === 'conversions') {
        ...
        <div className="bg-[#1a1a1a] rounded-lg overflow-hidden">
          <div className="p-4 border-b border-gray-800">
            <h3 className="text-gray-300 text-sm font-medium">BING ADS KEYWORD SUMMARIES (MOST RECENT WEEK)</h3>
          </div>
            ...
            <table className="w-full text-sm">
              <thead className="bg-black">
                <tr className="text-gray-400 text-xs">
                  ...
                  <th className="text-center p-3">CTR</th>
                ...
                {keywordNames.map((kw, i) => {
                  ...
                  return (
                    <tr key={i} className="border-t border-gray-800 hover:bg-gray-900">
                      ...
                      <td className="p-3 text-center text-gray-300">{latest.ctr.toFixed(1)}%</td>
                      <td className="p-3 text-right text-gray-300">${Math.round(latest.cost)}</td>
                      <td className="p-3 text-right text-gray-300">{Math.round(latest.conversions)}</td>
                      <td className="p-3 text-center text-gray-300">{latest.convRate.toFixed(1)}%</td>
                      <td className="p-3 text-right text-gray-300">${Math.round(latest.cpa)}</td>
                    </tr>
                  )
                })}
              </tbody>
