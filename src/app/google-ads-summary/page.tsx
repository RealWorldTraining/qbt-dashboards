            ...
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={chartData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                ...
                <Bar yAxisId="left" dataKey="spend" name="spend" fill={METRIC_COLORS.spend} radius={[4, 4, 0, 0]}>
                  <LabelList dataKey="spend" position="top" fill={METRIC_COLORS.spend} fontSize={11} formatter={(value: number) => typeof value === 'number' ? `$${(value / 1000).toFixed(0)}k` : ''} />
                </Bar>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={chartData} margin={{ top: 30, right: 30, left: 0, bottom: 0 }}>
                ...
                <Bar yAxisId="left" dataKey="cpa" name="cpa" fill={METRIC_COLORS.cpa} radius={[4, 4, 0, 0]}>
                  <LabelList dataKey="cpa" position="top" fill={METRIC_COLORS.cpa} fontSize={11} formatter={(value: number) => typeof value === 'number' ? `$${Math.round(value)}` : ''} />
                </Bar>
                <Line yAxisId="right" type="monotone" dataKey="avg_cpc" name="avg_cpc" stroke={METRIC_COLORS.avg_cpc} strokeWidth={3} dot={{ fill: METRIC_COLORS.avg_cpc, r: 5 }}>
                  <LabelList dataKey="avg_cpc" position="top" fill={METRIC_COLORS.avg_cpc} fontSize={11} offset={12} formatter={(value: number) => typeof value === 'number' ? `$${value.toFixed(2)}` : ''} />
                </Line>
              </ComposedChart>
            </ResponsiveContainer>
          )}
