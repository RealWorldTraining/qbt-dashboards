...
async function checkTab() {
  try {
    ...
    const targetSheet = response.data.sheets?.find(s => s.properties?.sheetId === 940763578);
    if (targetSheet) {
      console.log('Found tab:', targetSheet.properties?.title);
      
      // Now get data from that tab
      const range = `${targetSheet.properties?.title}!A1:AC5`;
      ...
      console.log('Tab with gid 940763578 not found');
    }
  } catch (error) {
